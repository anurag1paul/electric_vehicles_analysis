...
def build_model_data_dict(insurance_msrp, electricity_data, gas_price, co2_mwh, maintenance_data, fuel_economy_data, mpg_threshold = 70):
	'''
	...
	for car_info in insurance_msrp.iterrows():
		...
		if mpg == 0:
			brand_data = fuel_economy_data[fuel_economy_data['make'] == car_info[1]['Make']]
			best_score = -1
			best_match = ''
		...
		for state in state_list:
			for mileage in [60000, 120000, 180000]:
				...
				master_dict[car_name]['user_type'][mileage]['fuel_cost'][state] = fuel_cost
				master_dict[car_name]['user_type'][mileage]['env_cost'][state] = env_cost
...
def get_best_vehicle(df, price_range,fuel_type):
	...
	:return: DataFrame
	'''
	temp_df = df[df['base'] <= price_range[1]][df['base'] >= price_range[0]][df['fuel_type'] == fuel_type]
	op_cost = list()
